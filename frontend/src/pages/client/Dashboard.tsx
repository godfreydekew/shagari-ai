...
import { Leaf, FileText, ChevronRight, MessageCircle, Loader2 } from 'lucide-react';
    ...
    <div className="page-enter max-w-2xl mx-auto px-4 py-6 md:px-8 md:py-8 space-y-7">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-display font-semibold text-foreground">{season.season} in Your Garden</h3>
            <p className="text-sm text-muted-foreground mt-1 leading-relaxed">{season.tip}</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-display font-semibold text-foreground">My Garden</h2>
          {plants.length > 0 && (
            <Link to="/plants" className="text-sm text-primary font-medium flex items-center gap-1">
        ...
        {!isLoading && !error && plants.length === 0 && (
          <div className="card-botanical p-5 text-center">
            <Leaf className="mx-auto text-muted-foreground/20 mb-3" size={40} />
            <p className="text-sm text-muted-foreground">No plants in your garden yet</p>
            <p className="text-xs text-muted-foreground/70 mt-1">Your GardenKeeper team will add plants to your garden</p>
          </div>
        )}
                    ...
                    <div className="w-full aspect-square rounded-xl bg-secondary flex items-center justify-center mb-3">
                      {plant.image_url && !brokenImages[plant.id] ? (
                        <img
                          ...
                          onError={() => setBrokenImages(prev => ({ ...prev, [plant.id]: true }))}
                        />
                      ) : (
                        <Leaf className="text-primary/30" size={32} />
                      )}
                    </div>
                    <h3 className="text-sm font-semibold text-foreground truncate">{plant.common_name}</h3>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <Link to="/booklet" className="block card-botanical-interactive p-5 garden-gradient-soft">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-card flex items-center justify-center garden-shadow shrink-0">
              <FileText className="text-primary" size={22} />
            </div>
            <div className="flex-1">
              <h3 className="font-display font-semibold text-foreground">Your Garden Handbook</h3>
              <p className="text-sm text-muted-foreground mt-0.5">View or download your personalised plant guide</p>
            </div>
