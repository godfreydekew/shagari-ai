...
import { User, Mail, MapPin, Phone, Bell, LogOut, Lock, Trash2, Loader2, Pencil, Check, X } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 md:px-8 md:py-8 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="card-botanical p-6 text-center">
        ...
        {editingName ? (
          <div className="flex items-center justify-center gap-2">
            ...
            <button onClick={handleSaveName} disabled={savingName} className="text-primary hover:text-primary/80">
      ...
      <div className="card-botanical p-4">
        <button onClick={() => setShowPwForm(!showPwForm)} className="flex items-center gap-3 text-sm font-medium text-foreground w-full">
          <Lock size={18} className="text-primary" />
          Change Password
        </button>
        {showPwForm && (
          <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} onSubmit={handleChangePassword} className="mt-4 space-y-3">
      ...
      <div className="card-botanical p-4 border border-destructive/20">
        {!confirmDelete ? (
          <button onClick={() => setConfirmDelete(true)} className="flex items-center gap-3 text-sm font-medium text-destructive w-full">
