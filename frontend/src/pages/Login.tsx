...
export default function Login() {
  ...
  function resetForm() {
    ...
    setFullName(''); setGardenIdInput(''); setError(''); setShowPassword(false);
  }
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const user = await signup({ email, password, full_name: fullName, garden_code: gardenIdInput.trim() || undefined });
      navigate(user.role === 'admin' ? '/admin' : '/dashboard');
    } catch (err) {
      setError(getApiError(err));
    } finally {
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    setError('Password recovery is not available yet. Contact your Shagari administrator.');
  }
      ...
      >
        <div className="card-botanical p-8 md:p-10">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-display font-bold text-foreground">Shagari Garden Management</h1>
            <p className="text-sm text-muted-foreground mt-1">Your garden, always in bloom</p>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'signup' && (
              <motion.form key="signup" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Garden ID</label>
                  <input type="text" value={gardenIdInput} onChange={e => setGardenIdInput(e.target.value)} required
                    ...
                    placeholder="GK-XXXX" />
                  <p className="text-xs text-muted-foreground mt-1">Enter the Garden ID provided by your Shagari team</p>
                </div>
                {error && <p className="text-sm text-destructive">{error}</p>}
                <button type="submit" disabled={loading}
                  className="w-full pill-button garden-gradient text-primary-foreground font-semibold text-sm garden-shadow-lg hover:opacity-90 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">
            ...
            {view === 'forgot' && (
              <motion.form key="forgot" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <p className="text-sm text-muted-foreground text-center">Enter your email and we'll send a recovery link.</p>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">Email</label>
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                    className="w-full px-4 py-3 rounded-xl bg-secondary border-0 text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/30 focus:outline-none"
